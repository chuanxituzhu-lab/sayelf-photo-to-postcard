...
6. Track manual edits only to the four primary copy fields (Chinese title, English title, subtitle, and phrase). When one of these four fields has been manually edited, preserve it across later image uploads; location and date remain automatic metadata and update with each new image.
...
12. Treat the second reference postcard as the fixed design-system master. The selected art style applies only to the stylized image area; do not change the paper, frame, typography, ornaments, metadata layout, swatches, phrase, spacing, or source-photo area.
